...
test_that("Cannot retrieve trajectories until model is fitted", {
  ...
  expect_error(mod$population_trajectories(), "Model has not been fitted yet. Call 'fit' before calling this function.")
})

test_that("Can retrieve summarised trajectories", {
  mod <- scova$new(file_path = system.file("delta_full.rds", package = "epikinetics"))
  mod$fit()
  trajectories <- mod$population_trajectories(summarise = TRUE)
...
test_that("Can retrieve un-summarised trajectories", {
  mod <- scova$new(file_path = system.file("delta_full.rds", package = "epikinetics"))
  mod$fit()
  trajectories <- mod$population_trajectories(summarise = FALSE)
